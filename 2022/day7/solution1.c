...
/* returns the total size of the directory d (including sub-directories) fulfilling a condition */
long total_size_with_size_condition(dir *d, int (*check_dir)(dir *)) {
    if (d) {
        long size = ((*check_dir)(d)) ? total_size(d) : 0L;

        /* add the sub-directory sizes */
        for (int i = 0; i < d->no_subdirs; i++) {
            size += total_size_with_size_condition(d->subdirs[i], check_dir);
        }
...
/* checks if the total size of the directory is less than */
int chk_dir(dir *d) {
    return (d && total_size(d) <= 100000L);
}
...
int main(int argc, char *argv[]) {
    ...
    if ((fp = fopen(argv[1], "r")) != NULL) {
        ...
        printf("Sum of the total sizes: %ld\n", total_size_with_size_condition(root, chk_dir));
